...
class KMP:
    ...
    def add_user_text(self):
        ...
        if user_text:
            ...
            print(f"Текст успешно добавлен! (Длина: {len(user_text)} символов)")
        else:
    ...
    def python_search(self, pattern):
        """Поиск с использованием встроенного метода Python str.find()"""
        if not pattern:
            return [], 0.0
        ...
        for text_idx, text in enumerate(self.texts):
            ...
            pattern_lower = pattern.lower() if not self.case_sensitive else pattern

            pos = 0
            while True:
                pos = search_text.find(pattern_lower, pos)
                if pos == -1:
                    break
                results.append((text_idx, pos))
                pos += 1
    ...
    def show_stats(self):
        """Показывает статистику по добавленным текстам"""
        print("\nСтатистика текстов:")
        for i, text in enumerate(self.texts, 1):
            words = len(text.split())
            chars = len(text)
            print(f"Текст #{i}: {words} слов, {chars} символов")
...
def main():
    ...
    while True:
        ...
        print("2. Поиск подстроки (алгоритм КМП)")
        print("3. Поиск подстроки (встроенный поиск Python)")
        print("4. Сравнить оба алгоритма поиска")
        print("5. Изменить чувствительность к регистру")
        print("6. Показать все тексты")
        print("7. Статистика текстов")
        print("8. Выход")
        ...
        if choice == "1":
            ...
            if choice == "2":
                results, search_time = kmp.kmp_search(pattern)
                method_name = "КМП"
            elif choice == "3":
                results, search_time = kmp.python_search(pattern)
                method_name = "Python"
            else:  # choice == "4"
                # Сравнение обоих методов
                kmp_results, kmp_time = kmp.kmp_search(pattern)
                py_results, py_time = kmp.python_search(pattern)

                print("\nСравнение алгоритмов:")
                print(f"Алгоритм КМП: найдено {len(kmp_results)} совпадений за {kmp_time:.6f} сек")
                print(f"Встроенный поиск Python: найдено {len(py_results)} совпадений за {py_time:.6f} сек")
                print(f"Разница: {abs(kmp_time - py_time):.6f} сек")

                if kmp_results != py_results:
                    print("\nВнимание: алгоритмы дали разные результаты!")

                continue

            if not results:
                print(f"\nМетод {method_name}: подстрока не найдена.")
            else:
                print(f"\nМетод {method_name}: найдено {len(results)} совпадений за {search_time:.6f} сек:")
                for i, (text_idx, pos) in enumerate(results, 1):
                    text = kmp.texts[text_idx]
                    start = max(0, pos - 20)
            ...
            sensitive = input("Чувствительность к регистру (y/n): ").lower().strip() == 'y'
            kmp.set_case_sensitive(sensitive)
        elif choice == "6":
            print("\nВсе тексты:")
            for i, text in enumerate(kmp.texts, 1):
                print(f"\nТекст #{i} ({len(text)} символов):")
